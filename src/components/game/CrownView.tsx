  ...
  return (
    <div className="absolute inset-0 flex flex-col overflow-y-auto bg-bg px-4 pb-28 pt-24">
      ...
      <p className="mt-2 text-sm leading-relaxed text-muted">
        Bells against bodies. When the people stand at fifty, you may declare. Then you must win the
        war.
      </p>
      ...
      {state.war && !state.war.resolved ? (
        <div className="mt-4 rounded-[var(--radius-lg)] border border-bad/50 bg-surface p-4">
          <p className="font-display text-lg">
            {state.war.kind === "revolution" ? "War of independence" : "Punitive raid"}
          </p>
          <p className="mt-1 text-sm text-muted">
            Enemy host {state.war.enemy} · landfall in {state.war.eta} days. Train militia in the
            barracks.
          </p>
        </div>
      ) : null}
